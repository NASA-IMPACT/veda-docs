...
Additional [Cloud-Optimized GeoTIFFs details](https://guide.cloudnativegeo.org/cloud-optimized-geotiffs/intro.html) that can be helpful.
...
Additional [Zarr details](https://guide.cloudnativegeo.org/zarr/intro.html) that can be helpful.
...
Additional [Cloud-Optimized HDF/NetCDF details](https://guide.cloudnativegeo.org/cloud-optimized-netcdf4-hdf5/) that can be helpful.
### **Vector Data**
...
Additional [GeoParquet](https://guide.cloudnativegeo.org/geoparquet/) that can be helpful.
...
This approach aligns with NASA ESDS guidance on “balanced chunking” for multidimensional datasets.
...
- Use **DEFLATE (gzip)** compression (recommended for interoperability across NetCDF4/HDF5 tools)  
- Use a moderate compression level (e.g., **~5**) to balance file size and performance  
- Apply a **shuffle filter** to improve compression efficiency  

> **Note:** NASA ESDS guidance recommends using only officially supported compression filters (e.g., DEFLATE) to ensure compatibility across tools and platforms.  
> *DIWG, “Use Only Officially Supported Compression Filters on NetCDF-4 and NetCDF-4-Compatible HDF5 Data,” January 10, 2024.*
...
- Tune chunk sizes appropriately:
  - Chunk size should be selected based on expected access patterns  
  - In cloud-based workflows, **~1–10 MB per chunk** is commonly recommended to balance request overhead and data transfer efficiency  
...
### **Data Packing (Optional Optimization)**

- Store data using **scale_factor** and **add_offset** to reduce size  
- Convert floating-point data to lower-precision integers where appropriate  
- Improves compression efficiency when combined with shuffle + deflate  

**Note:** Recommended when data has a limited dynamic range.

---

### **Useful Tools**

- `nccopy` — rechunking and compression (NetCDF)
- `ncks` — data transformation and packing
- `h5repack` — optimize HDF5 chunking/compression