...
If you already have a robust web presence but want to use VEDA's powerful **tools** and **widgets**—such as data stories or analysis tools, then you can simply import them as an NPM package. The package is published under `@teamimpact/veda-ui`.
...
Currently available components, hooks and providers:

Providers and Hooks: `DevseedUiThemeProvider`, `VedaUIProvider`, `ReactQueryProvider`, `useTimelineDatasetAtom`, `useFiltersWithQS`

UI Components: `Block`, `Image`, `MapBlock`, `ScrollytellingBlock`, `Figure`, `Prose`, `MDXImage`, `Caption`, `Chapter`, `Chart`, `Table`, `CompareImage`, `Embed`, `PageHeader`, `PageFooter`, `DefaultCard`, `FlagCard`, `Widget`, `PageHero`

Page Components: `CatalogContent`, `ExplorationAndAnalysis`, `StoriesHubContent`, `DatasetSelectorModal`

**Note:** VEDA is in the process of migrating the remaining components to the USWDS system. If you need additional UI primitives to build custom components within VEDA, we recommend using [USWDS](https://designsystem.digital.gov/) or [react-uswds](https://github.com/trussworks/react-uswds) (if using React).
