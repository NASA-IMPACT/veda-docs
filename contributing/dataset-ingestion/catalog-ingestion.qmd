...
3. [VEDA] Publishing to the production STAC catalog `https://openveda.cloud`
...
You can obtain a token by clicking the 'Authorize' in the workflows API swagger docs at [staging.openveda.cloud/api/workflows/docs](https://staging.openveda.cloud/api/workflows/docs). It will be added to the headers of all requests made from the swagger docs page. 

Alternatively, you can use the cognito client to obtain a token for your request headers.
```python
# Required imports
import os
import requests
...
STAGING_WORKFLOWS_API_URL = "https://staging.openveda.cloud/api/workflows"

# url for dataset validation / publication
validate_url = f"{STAGING_WORKFLOWS_API_URL}/dataset/validate"

publish_url = f"{STAGING_WORKFLOWS_API_URL}/dataset/publish"
...
execution_status_url = f"{STAGING_WORKFLOWS_API_URL}/discovery-executions/{execution_id}"
# make the request
